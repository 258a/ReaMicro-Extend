    ...
    private inner class Bridge {
        ...
        fun setBanner(path: String?): String = safeJson {
            ...
            if (imageFile != targetFile) {
                if (target.exists()) require(target.delete()) { "横幅文件已存在且无法替换" }
                ...
                if (coverFile != null && imageFile == coverFile) {
                    ...
                    image.copyTo(target, overwrite = true)
                    require(image.delete()) { "横幅已复制但原文件无法移除" }
                }
            }
...
.highlight{color:#20242d;background:transparent;pointer-events:none;z-index:1;display:none}
.highlight:empty::before{content:" "}
.editor-stack.no-wrap .highlight,.editor-stack.no-wrap .code-input,.editor-stack.no-wrap .search-overlay{white-space:pre;word-break:normal}
.editor-stack.highlight-ready .highlight{display:block}.editor-stack.highlight-ready .code-input{color:transparent;-webkit-text-fill-color:transparent}
.search-overlay{background:transparent;pointer-events:none;z-index:4;scrollbar-width:none}
...
.code-input{resize:none;background:transparent;color:#20242d;caret-color:#20242d;-webkit-text-fill-color:#20242d;z-index:3}
...
const state={title:"",status:"",metadata:{},files:[],groups:{},open:{},editing:null,content:"",dirty:false,matches:[],matchIndex:-1,actionFile:null,renameFile:null,searchResults:[],searchResultIndex:-1,decorating:false,searchDirty:true,renderVersion:0,layoutVersion:-1,layoutNodes:[],layoutLength:0,layoutFrame:0,editorSyncFrame:0,highlightTimer:0,highlightToken:0,openSearchToken:0,scopeJobToken:0,searchSessionToken:0,treeScrollTop:0,lineOffsetCacheText:null,lineOffsetCache:[0],lineRenderStart:-1,lineRenderEnd:-1,lineRenderCount:-1,page:"tree",coverPickerArmed:0,coverPicking:false,coverPickerBlockedUntil:0};
// Large EPUB metadata files must be editable immediately; syntax highlight is delayed and skipped past this size.
const HIGHLIGHT_MAX_CHARS=90000;
...
function scheduleLayoutSync(after){if(state.layoutFrame)cancelAnimationFrame(state.layoutFrame);state.layoutFrame=requestAnimationFrame(()=>{state.layoutFrame=0;syncLines(true);syncSearchHits();syncScroll();if(typeof after==="function")after()})}
...
function isEditorNoWrap(){return !!byId("editorBody")?.querySelector(".editor-stack.no-wrap")}
function syncLines(force=false){if(isEditorNoWrap())syncPlainLines(force);else syncWrappedLines(force)}
function syncPlainLines(force=false){const text=byId("text"),lineCanvas=byId("lineCanvas");if(!text||!lineCanvas)return;const metrics=contentMetrics(text);const lineHeight=metrics.pad.lineHeight;const offsets=ensureLineOffsets(text.value);const total=Math.max(1,offsets.length);lineCanvas.style.height=Math.max(metrics.height,metrics.viewportHeight,total*lineHeight)+"px";const first=Math.max(0,Math.floor(text.scrollTop/lineHeight)-30);const last=Math.min(total-1,Math.ceil((text.scrollTop+text.clientHeight)/lineHeight)+30);if(!force&&first===state.lineRenderStart&&last===state.lineRenderEnd&&total===state.lineRenderCount)return;state.lineRenderStart=first;state.lineRenderEnd=last;state.lineRenderCount=total;const rows=[];for(let i=first;i<=last;i++){rows.push('<div class="line-row" style="top:'+(i*lineHeight)+'px;height:'+lineHeight+'px">'+String(i+1)+'</div>')}lineCanvas.innerHTML=rows.join("")}
function syncWrappedLines(force=false){const text=byId("text"),lineCanvas=byId("lineCanvas");if(!text||!lineCanvas)return;const metrics=contentMetrics(text);lineCanvas.style.height=Math.max(metrics.height,metrics.viewportHeight,metrics.pad.lineHeight)+"px";if(!byId("highlight")?.parentElement?.classList.contains("highlight-ready")){lineCanvas.innerHTML="";return}const first=Math.max(0,Math.floor(text.scrollTop/metrics.pad.lineHeight)-30);const last=Math.ceil((text.scrollTop+text.clientHeight)/metrics.pad.lineHeight)+30;const offsets=ensureLineOffsets(text.value);if(!force&&first===state.lineRenderStart&&last===state.lineRenderEnd&&offsets.length===state.lineRenderCount)return;state.lineRenderStart=first;state.lineRenderEnd=last;state.lineRenderCount=offsets.length;const minTop=first*metrics.pad.lineHeight,maxTop=last*metrics.pad.lineHeight,rows=[];for(let i=0;i<offsets.length;i++){const start=offsets[i];const end=i+1<offsets.length?Math.max(start,offsets[i+1]-1):text.value.length;measureOffsetRects(start,end).forEach((rect,index)=>{if(rect.top+rect.height<minTop||rect.top>maxTop)return;rows.push('<div class="line-row" style="top:'+rect.top+'px;height:'+Math.max(rect.height,metrics.pad.lineHeight)+'px">'+(index===0?String(i+1):"")+'</div>')})}lineCanvas.innerHTML=rows.join("")}
function syncScroll(){const text=byId("text"),hi=byId("highlight"),overlay=byId("searchOverlay"),lines=byId("lines"),lineCanvas=byId("lineCanvas");if(!text)return;syncLines(false);if(hi){hi.scrollTop=text.scrollTop;hi.scrollLeft=text.scrollLeft}if(overlay){overlay.scrollTop=text.scrollTop;overlay.scrollLeft=text.scrollLeft}if(lines&&lineCanvas){lineCanvas.style.transform='translateY('+-text.scrollTop+'px)'}updateMatchLine()}
function scheduleHighlight(delay){clearTimeout(state.highlightTimer);const text=byId("text");if(!text)return;const token=++state.highlightToken;state.highlightTimer=setTimeout(()=>{if(token!==state.highlightToken)return;syncHighlight();syncScroll()},delay)}
function syncHighlight(){const text=byId("text"),hi=byId("highlight"),stack=hi?.parentElement;if(!text||!hi)return;const value=text.value||"";if(value.length>HIGHLIGHT_MAX_CHARS){hi.textContent="";if(stack)stack.classList.remove("highlight-ready");state.renderVersion++;state.layoutVersion=-1;state.layoutNodes=[];state.layoutLength=0;return}hi.innerHTML=highlightCode(value,state.editing||{});if(stack)stack.classList.add("highlight-ready");state.renderVersion++;state.layoutVersion=-1;state.layoutNodes=[];state.layoutLength=0}
...
function lineIndexForOffset(offset){const text=byId("text");if(!text)return 0;const offsets=ensureLineOffsets(text.value);const safe=Math.max(0,Math.min(offset,text.value.length));let low=0,high=offsets.length-1,match=0;while(low<=high){const mid=(low+high)>>1;if(offsets[mid]<=safe){match=mid;low=mid+1}else high=mid-1}return match}
function measureCaretTop(offset){const text=byId("text"),hi=byId("highlight");if(!text||!hi)return 0;const metrics=contentMetrics(text);const range=buildDomRange(offset,offset);if(!range)return Math.max(0,Math.min(text.scrollHeight,lineIndexForOffset(offset)*metrics.pad.lineHeight));const rect=range.getBoundingClientRect();const hiRect=hi.getBoundingClientRect();return Math.max(0,rect.top-hiRect.top-metrics.pad.top+hi.scrollTop)}
...
function findRanges(value,pattern){const ranges=[];if(!pattern)return ranges;if(pattern instanceof RegExp){pattern.lastIndex=0;let m;while((m=pattern.exec(value))){if(!m[0]){pattern.lastIndex++;continue}ranges.push([m.index,m.index+m[0].length])}pattern.lastIndex=0;return ranges}const source=value.toLowerCase();const needle=pattern.text.toLowerCase();let p=0;while((p=source.indexOf(needle,p))!==-1){ranges.push([p,p+needle.length]);p+=needle.length||1}return ranges}
...
function scrollSelectionIntoView(text,start,end){const rects=measureSearchRects(start,end);const rect=rects[0];if(!rect){const lineIndex=lineIndexForOffset(start);const lineHeight=getBoxPadding(text).lineHeight;text.scrollTop=Math.max(0,lineIndex*lineHeight);syncScroll();return}const overlayHeight=byId("replace")?.classList.contains("open")?(byId("replace").offsetHeight||0):0;const viewportTop=text.scrollTop;const viewportBottom=text.scrollTop+Math.max(140,text.clientHeight-overlayHeight-16);const targetTop=Math.max(0,rect.top-12);const targetBottom=rect.top+rect.height+12;if(targetTop<viewportTop){text.scrollTop=targetTop}else if(targetBottom>viewportBottom){text.scrollTop=Math.max(0,targetBottom-(text.clientHeight-overlayHeight)*0.62)}const viewportLeft=text.scrollLeft;const viewportRight=text.scrollLeft+Math.max(80,text.clientWidth-24);const targetLeft=Math.max(0,rect.left-18);const targetRight=rect.left+rect.width+18;if(targetLeft<viewportLeft){text.scrollLeft=targetLeft}else if(targetRight>viewportRight){text.scrollLeft=Math.max(0,targetRight-text.clientWidth*0.72)}syncScroll()}
...
function findNext(){const scope=byId("scope").value;if(state.searchDirty)buildMatches(false);if(scope==="current"){if(!state.matches.length)return;state.matchIndex=(state.matchIndex+1)%state.matches.length;selectMatch();return}if(!state.searchResults.length)return;if(state.searchResultIndex<0){openSearchResult(0,0);return}const current=state.searchResults[state.searchResultIndex];if(current&&state.editing?.path!==current.path){openSearchResult(state.searchResultIndex,Math.max(0,state.matchIndex));return}const currentMatch=state.matches===current?.matches?state.matchIndex:-1;if(current&&currentMatch+1<current.matches.length){state.matches=current.matches;state.matchIndex=currentMatch+1;selectMatch();return}const nextFile=(state.searchResultIndex+1)%state.searchResults.length;openSearchResult(nextFile,0)}
function findPrev(){const scope=byId("scope").value;if(state.searchDirty)buildMatches(false);if(scope==="current"){if(!state.matches.length)return;state.matchIndex=(state.matchIndex-1+state.matches.length)%state.matches.length;selectMatch();return}if(!state.searchResults.length)return;if(state.searchResultIndex<0){const last=state.searchResults.length-1;openSearchResult(last,state.searchResults[last].matches.length-1);return}const current=state.searchResults[state.searchResultIndex];if(current&&state.editing?.path!==current.path){openSearchResult(state.searchResultIndex,Math.max(0,state.matchIndex));return}const currentMatch=state.matches===current?.matches?state.matchIndex:current?.matches.length||0;if(current&&currentMatch>0){state.matches=current.matches;state.matchIndex=currentMatch-1;selectMatch();return}const prevFile=(state.searchResultIndex-1+state.searchResults.length)%state.searchResults.length;const prevItem=state.searchResults[prevFile];openSearchResult(prevFile,prevItem.matches.length-1)}
...
function setBanner(path){const r=callJson(()=>api.setBanner(path));toast(r.message||"已设为横幅");hideSheet();if(r.ok)refresh([path,r.path])}
